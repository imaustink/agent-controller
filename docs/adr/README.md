...
| [0025](0025-triage-acknowledgment-comment-and-session-viewer.md) | A triaged GitHub issue gets an upfront "I'm starting to look into this" comment before the (possibly long) agent turn runs, optionally linking to a new session-viewer page (`GET/POST /sessions/:sessionId`) for watching the transcript and sending the agent additional instructions |
