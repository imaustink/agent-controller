...
> **Implementation status:** Phase 1 (the `faas` adapter / direct-by-id
> invocation surface only) is implemented in
> [apps/integration-gateway](../apps/integration-gateway/) — see that app's
> README for the current `POST /fn/:id` / `GET /fn/runs/:id` contract. The
> conversational (Slack/email) adapters and the `IntegrationRoute`
> routing-config open question below remain unimplemented.
...
- `externalThreadId` maps 1:1 onto the existing `session_id` concept — the
  ...
  `checkActiveSkill`, or continuation tokens (ADR 0017); a Slack thread or an
  email chain gets the same session-scoped active-skill and continuation
...
launch it directly" instead of "an LLM chose which CR to launch."
...
- **Polling vs. push for gateway → adapter results.** `POST /invoke`'s async
  ...
  *from the gateway itself* (a trusted first-party caller, not the
  caller-supplied-webhook SSRF risk ADR 0006 rejected for end users).
...
2. **One adapter for the conversational path** (Slack is the natural choice —
   well-documented Events API, built-in signature verification, thread
   semantics map cleanly to `session_id`): validates the
   `InboundEvent`/`SessionStore` integration end to end.
3. **Email adapter**: highest-risk channel, built once the injection/
   untrusted-content handling patterns are proven out on Slack.
