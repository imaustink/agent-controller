...
> Source: [github.com/imaustink/agent-controller](https://github.com/imaustink/agent-controller)
...
| Concern | Without a controller | With agent-controller |
| ------- | -------------------- | ---------------------- |
...
docker build -f controllers/tool-controller/Dockerfile   -t tool-controller:latest controllers/tool-controller/
```
...
```sh
kubectl apply -f charts/tool-controller/crds/
```
...
> `make manifests`, copy the generated CRD to `charts/tool-controller/crds/`,
> then `kubectl apply -f charts/tool-controller/crds/<changed>.yaml` by hand.

### Deploying

```sh
make deploy IMG=<registry>/tool-controller:tag
```
