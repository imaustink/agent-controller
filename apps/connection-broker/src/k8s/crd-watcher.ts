...
/*
 * DUPLICATED from apps/agent-orchestrator/src/k8s/crd-watcher.ts.
 *
 * Copied rather than shared because the broker is a deliberately separate
 * Deployment (ADR 0038 §3) — third-party credentials and outbound egress stay
 * out of the pod holding cluster RBAC — and there is no package today that both
 * could depend on without one pulling in the other's dependency tree. If a
 * third consumer appears, this is the point to extract a package instead.
 *
 * Keep the two in step: the reconnect behaviour below is not incidental.
 */
