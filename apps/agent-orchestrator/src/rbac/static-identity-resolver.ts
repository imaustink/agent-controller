 ...
 * roles) is an explicitly open question — see
 * docs/orchestrator.md#open-questions-explicitly-deferred and ADR 0004. This
 * class exists so the rest of the orchestrator (RAG filtering, Job launch
 * scoping) can be built and tested against the {@link IdentityResolver} port
...
/**
 * Parses the `AGENT_STATIC_IDENTITIES` env var: JSON object of
 * `{ "<token>": { "subject": "...", "roles": ["..."] } }`. Returns an empty
 * map (fail closed) on missing/invalid input.
 */
  ...
  try {
    ...
    for (const [token, value] of Object.entries(parsed as Record<string, unknown>)) {
      if (
        ...
        const roles = (value as { roles: unknown[] }).roles.filter((r): r is string => typeof r === "string");
