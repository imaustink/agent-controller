...
{{/*
Predicted fullname of the bundled Open WebUI subchart's resources (mirrors
its own "open-webui.fullname" helper exactly, using our lowercase
"openwebui" alias in place of the chart name: release name as-is if it
already contains "openwebui", else "<release>-openwebui"). Only used for
NOTES.txt guidance -- the subchart still renders its own resource names
independently, this just predicts them so we don't have to guess in docs.
*/}}
{{- define "agent-orchestrator.openWebUIFullname" -}}
{{- $name := "openwebui" }}
{{- if contains $name .Release.Name }}
{{- .Release.Name }}
{{- else }}
