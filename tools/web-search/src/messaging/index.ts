...
import type { SearchErrorCode, SearchStage } from "../schema.js";
 ...
 * result (a rendered list of search results, built in index.ts -- same
 * "render to a string, don't hand back a raw object" convention every other
 * tool uses for its own output), pipeline {@link SearchStage}s, and
 * {@link SearchErrorCode}s. Same thin-wiring pattern as
 * tools/recipe-publisher/src/messaging/index.ts.
 */
export class JobEmitter extends BaseJobEmitter<string, SearchStage, SearchErrorCode> {
  constructor(jobId: string, sink: Sink<string>) {
    super(jobId, sink, { sanitize: clip });
  }
}

/** Selects the event transport from configuration (same shape as recipe-publisher). */
